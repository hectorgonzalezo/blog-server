...
exports.get_all_comments = (req: ExtendedRequest, res: Response, next: NextFunction) => {
...
exports.create_comment = [
  body("content", "Blog content is required")
    ...
    .escape(),
  body("poster", "Blog commenter is required")
    .trim()
    .escape(),
  (req: ExtendedRequest, res: Response, next: NextFunction) => {
    ...
    // If data is valid
    // Create new comment
    const reqBody = req.body as Pick<IComment, "content" | "published" | "poster" | "post" >;
    ...
    newComment.save((err) => {
      ...
      Post.findByIdAndUpdate(
        ...
        (updateErr, updatedPost) => {
          ...
          Post.findById(req.postId).populate({ path: "comments", populate: { path: "commenter" } }).exec((postErr,  post) => {
            if (postErr) {
              return next(postErr);
            }
